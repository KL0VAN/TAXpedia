...
/* ========================= CONFIG ========================= */
const TAX_BRACKETS = [
  { upper: 12096, type: "zero" },
  { upper: 17443, type: "y", base: 12096, a: 932.30, b: 1400 },
  { upper: 68480, type: "z", base: 17443, a: 176.64, b: 2397, add: 1015.13 },
  { upper: 277825, type: "linear", rate: 0.42, sub: 10911.92 },
  { upper: Infinity, type: "linear", rate: 0.45, sub: 19246.67 },
];


// Percorsi ai file di testo - aggiornare i nomi/file se necessario
const SETTORI = [
  { id: 'welf', nome:'Protezione sociale',               percentuale:0.405, link:'' },
  { id: 'def',  nome:'Difesa',                           percentuale:0.130, link:'' },
  { id: 'pa',   nome:'Pubblica amministrazione',         percentuale:0.089, link:'' },
  { id: 'ue',   nome:'Bilancio UE',                      percentuale:0.063, link:'' },
  { id: 'eco',  nome:'Politiche economiche',             percentuale:0.062, link:'' },
  { id: 'edu',  nome:'Istruzione e ricerca',             percentuale:0.058, link:'' },
  { id: 'inf',  nome:'Infrastrutture e trasporti',       percentuale:0.058, link:'' },
  { id: 'int',  nome:'Interessi',                        percentuale:0.056, link:'' },
  { id: 'san',  nome:'Sanità',                           percentuale:0.039, link:'' },
  { id: 'sic',  nome:'Sicurezza e ordine pubblico',      percentuale:0.016, link:'' },
  { id: 'amb',  nome:'Protezione ambientale',            percentuale:0.005, link:'' },
  { id: 'giu',  nome:'Giustizia',                        percentuale:0.002, link:'' },
  { id: 'cul',  nome:'Cultura e sport',                  percentuale:0.001, link:'' },
  { id: 'alt',  nome:'Altro',                            percentuale:0.016, link:'' }
];

const SETTORI_UE = [
  { nome:'Coesione, resilienza e valori',                percentuale:0.0247, link:'/article/europe/coesione.php' },
  { nome:'Risorse naturali e ambiente',                  percentuale:0.0179, link:'/article/europe/risorse_naturali.php' },
  { nome:'Mercato unico, innovazione e agenda digitale', percentuale:0.0068, link:'/article/europe/mercato_unico.php' },
  { nome:'Rapporti con il vicinato e con il mondo',      percentuale:0.0053, link:'/article/europe/rapporti_vicinato.php' },
  { nome:'Pubblica amministrazione europea',             percentuale:0.0040, link:'/article/europe/pubblica_amm_ue.php' },
  { nome:'Strumenti speciali',                           percentuale:0.0020, link:'/article/europe/strumenti_speciali.php' },
  { nome:'Migrazione e gestione delle frontiere',        percentuale:0.0015, link:'/article/europe/migrazione_gestione.php' },
  { nome:'Sicurezza e Difesa',                           percentuale:0.0008, link:'/article/europe/sicurezza_difesa.php' },
];
...
function calcolaIRPEF(reddito) {
  // Germania (2025) — solo imposta sul reddito (Einkommensteuer) su imponibile (zvE)
  // Implementa le formule ufficiali del Grundtarif (single) in base a §32a EStG (VZ 2025).
  // NB: il calcolo si fa su zvE "auf volle Euro abgerundet" (arrotondato per difetto all'euro).
  const x = Math.floor(Math.max(0, Number(reddito) || 0));

  const zone = TAX_BRACKETS.find(z => x <= z.upper) || TAX_BRACKETS[TAX_BRACKETS.length - 1];
  let tax = 0;
  ...
  switch (zone.type) {
    ...
    case 'y': {
      const y = (x - zone.base) / 10000;
      tax = (zone.a * y + zone.b) * y;
      break;
    }

    case 'z': {
      const z = (x - zone.base) / 10000;
      tax = (zone.a * z + zone.b) * z + zone.add;
      break;
    }

    case 'linear':
      tax = zone.rate * x - zone.sub;
      break;
  ...
  // ESt in euro, per difetto (coerente con "in Euro" del §32a)
