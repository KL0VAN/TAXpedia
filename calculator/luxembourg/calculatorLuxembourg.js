...
/* ========================= CONFIG ========================= */
const TAX_BRACKETS = [
  { min: 0,      max: 13230,  rate: 0.00 },
  { min: 13230,  max: 15435,  rate: 0.08 },
  { min: 15435,  max: 17640,  rate: 0.09 },
  { min: 17640,  max: 19845,  rate: 0.10 },
  { min: 19845,  max: 22050,  rate: 0.11 },
  { min: 22050,  max: 24255,  rate: 0.12 },
  { min: 24255,  max: 26550,  rate: 0.14 },
  { min: 26550,  max: 28845,  rate: 0.16 },
  { min: 28845,  max: 31140,  rate: 0.18 },
  { min: 31140,  max: 33435,  rate: 0.20 },
  { min: 33435,  max: 35730,  rate: 0.22 },
  { min: 35730,  max: 38025,  rate: 0.24 },
  { min: 38025,  max: 40320,  rate: 0.26 },
  { min: 40320,  max: 42615,  rate: 0.28 },
  { min: 42615,  max: 44910,  rate: 0.30 },
  { min: 44910,  max: 47205,  rate: 0.32 },
  { min: 47205,  max: 49500,  rate: 0.34 },
  { min: 49500,  max: 51795,  rate: 0.36 },
  { min: 51795,  max: 54090,  rate: 0.38 },
  { min: 54090,  max: 117450, rate: 0.39 },
  { min: 117450, max: 176160, rate: 0.40 },
  { min: 176160, max: 234870, rate: 0.41 },
  { min: 234870, max: Infinity, rate: 0.42 }
];

// Percorsi ai file di testo - aggiornare i nomi/file se necessario
const SETTORI = [
  { id: 'welf', nome:'Protezione sociale',               percentuale:0.243, link:'' },  
  { id: 'edu',  nome:'Istruzione e ricerca',             percentuale:0.172, link:'' },
  { id: 'inf',  nome:'Infrastrutture e trasporti',       percentuale:0.108, link:'' },
  { id: 'sic',  nome:'Sicurezza e ordine pubblico',      percentuale:0.097, link:'' },
  { id: 'san',  nome:'Sanità',                           percentuale:0.088, link:'' },
  { id: 'pa',   nome:'Pubblica amministrazione',         percentuale:0.068, link:'' },
  { id: 'eco',  nome:'Politiche economiche',             percentuale:0.056, link:'' },
  { id: 'def',  nome:'Difesa',                           percentuale:0.047, link:'' },
  { id: 'ue',   nome:'Bilancio UE',                      percentuale:0.016, link:'' },
  { id: 'amb',  nome:'Protezione ambientale',            percentuale:0.016, link:'' },
  { id: 'giu',  nome:'Giustizia',                        percentuale:0.012, link:'' },
  { id: 'cul',  nome:'Cultura e sport',                  percentuale:0.012, link:'' },
  { id: 'int',  nome:'Interessi',                        percentuale:0.010, link:'' },
  { id: 'alt',  nome:'Altro',                            percentuale:0.055, link:'' }
];

const SETTORI_UE = [
  { nome:'Coesione, resilienza e valori',                percentuale:0.006272, link:'/article/europe/coesione.php' },
  { nome:'Risorse naturali e ambiente',                  percentuale:0.004544, link:'/article/europe/risorse_naturali.php' },
  { nome:'Mercato unico, innovazione e agenda digitale', percentuale:0.001728, link:'/article/europe/mercato_unico.php' },
  { nome:'Rapporti con il vicinato e con il mondo',      percentuale:0.001344, link:'/article/europe/rapporti_vicinato.php' },
  { nome:'Pubblica amministrazione europea',             percentuale:0.001024, link:'/article/europe/pubblica_amm_ue.php' },
  { nome:'Strumenti speciali',                           percentuale:0.000512, link:'/article/europe/strumenti_speciali.php' },
  { nome:'Migrazione e gestione delle frontiere',        percentuale:0.000384, link:'/article/europe/migrazione_gestione.php' },
  { nome:'Sicurezza e Difesa',                           percentuale:0.000192, link:'/article/europe/sicurezza_difesa.php' },
];
...
function calcolaIRPEF(reddito) {
  // Lussemburgo (2025) — impôt sur le revenu, solo barème di base (classe 1),
  // senza maggiorazioni / Fonds pour l'emploi.
  const x = Math.floor(Math.max(0, Number(reddito) || 0));
  ...
  for (const b of TAX_BRACKETS) {
    ...
    const imponibileScaglione = Math.min(x, b.max) - b.min;
    if (imponibileScaglione > 0) {
      impostaBase += imponibileScaglione * b.rate;
    }
  }

  // solo imposta calcolata con le aliquote per scaglioni
  return Math.floor(Math.max(0, impostaBase));
}
...
function runCalcolo() {
  ...
  const rowsIt = SETTORI.map(s => ({
    nome: `<a href="${window.tpLangUrl ? window.tpLangUrl(s.link) : s.link}" target="_blank" rel="noopener noreferrer" style="color:inherit;text-decoration:underline">${tpCalcLabel(s.nome)}</a>`,
